...
lapply(c('lems', 'uems', 'ltscore', 'ppscore', 'modben'), function(outcome){
  ...
  data_outcome_imputed <- read_xlsx(paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - ", min_non_NA, "\\data_imputed_", outcome, ".xlsx"))
  ...
  write_xlsx(data, paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - ", min_non_NA, "\\data_imputed_", outcome, "_renamed.xlsx"))
  ...
  lapply(c("lpa", "kmeans"), function(meth){
    imp_list <- lapply(1:20, function(imp){
      G_list <- lapply(2:6, function(G){
        data <- as.matrix(data_outcome_imputed[data_outcome_imputed$.imp == imp, out_cols[[outcome]]])
        clusters <- unlist(data_outcome_imputed[data_outcome_imputed$.imp == imp, c(paste0(meth, "-clusters-", G))])
    ...
    write_xlsx(bind_rows(imp_list), paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - ", min_non_NA, "\\", meth, "_quality_criteria_imputed_", outcome, "_renamed.xlsx"))
  })
})
