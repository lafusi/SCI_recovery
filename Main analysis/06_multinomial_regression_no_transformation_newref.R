...
lapply(c('lems', 'uems', 'ltscore', 'ppscore', 'modben'), function(outcome){
  ...
  mm <- lapply(c("lpa", "kmeans"), function(meth){
    gg <- lapply(2:6, function(g){
      ...
      summary_nomed$term <- as.character(summary_nomed$term)
      summary_med$term <- as.character(summary_med$term)
  ...
  write_xlsx(imp_res_med, paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - ", min_non_NA, "\\multinom_results_med_", outcome, "_2_no_transformation_newref.xlsx"))
  write_xlsx(imp_res_nomed, paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - ", min_non_NA, "\\multinom_results_nomed_", outcome, "_2_no_transformation_newref.xlsx"))
})