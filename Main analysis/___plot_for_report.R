...
plots <- lapply(c('lems', 'uems', 'ltscore', 'ppscore', 'modben'), function(outcome){
  ...
  ggplot() + geom_line(mapping = aes(x = week, y = estimate, color = as.character(alloc)), data = dat, linewidth = 1) + geom_ribbon(mapping = aes(x = week, ymin = estimate - std.error, ymax = estimate + std.error, fill = as.character(alloc)), data = dat, alpha = 0.3, color = NA) + 
    ...
    scale_color_manual(values = colors_mean[1:ncl[outcome]]) + scale_fill_manual(values = colors_mean[1:ncl[outcome]]) + labs(x = "Time after Injury [Weeks]", y = label[outcome]) + 
...
#png(file="C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\plot_for_report-.png" ,width=500, height=2500)
#plot_grid(plotlist = plots, labels = "AUTO", ncol = 1, label_size = 30, label_x = -0.01)
#dev.off()

#png(file="C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\plot_for_report_2-.png" ,width=1000, height=1500)
#plot_grid(plotlist = plots, labels = "AUTO", ncol = 2, label_size = 30, label_x = -0.01)
#dev.off()

#png(file="C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\plot_for_report_3-.png" ,width=1500, height=1000)
#plot_grid(plotlist = plots, labels = "AUTO", ncol = 3, label_size = 30, label_x = -0.01)
#dev.off()

png(file="C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\plot_for_report_3b-NEW.png" ,width=1200, height=800)
plot_grid(plotlist = plots, labels = "AUTO", ncol = 3, label_size = 30, label_x = -0.02) #, label_x = -0.05, scale = 0.95
dev.off()
...
png(file="C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\common_legend_NEW.png" ,width=200, height=100)
grid.newpage()
grid.draw(plot_legends[[2]])
dev.off()

p <- image_read("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\plot_for_report_3b-NEW.png")
l <- image_read("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\common_legend_NEW.png")

image_write(image_composite(p, l, offset = "+900+550"), "C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Main analysis\\results - 2\\plot_with_legend_NEW.png")
