...
lapply(c('lems', 'uems', 'ltscore', 'ppscore', 'modben'), function(outcome){
  ...
  mm <- lapply(c("lpa", "kmeans"), function(meth){
    gg <- lapply(2:6, function(g){
      ...
      summary_ci_OR_nomed <- summary_ci_nomed[, c('y.level', 'term', 'estimate', 'std.error', 'statistic', 'p.value')]
      ...
      summary_ci_OR_med <- summary_ci_med[, c('y.level', 'term', 'estimate', 'std.error', 'statistic', 'p.value')]
  ...
  mm <- lapply(c("lpa", "kmeans"), function(meth){
    gg <- lapply(2:6, function(g){
      ...
      df_list <- lapply(list(model_nomed, model_med), function(mod){
        ...
        df_reshaped <- df_reshaped[, c('y.level', 'term', 'estimate', 'std.error', 'statistic', 'p.value')]
  ...
  write_xlsx(all_res_med, paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Sensitivity analysis\\data_sensitivity_analysis - ", min_non_NA, "\\multinom_results_med_", outcome, "_2_no_transformation_newref.xlsx"))
  write_xlsx(all_res_nomed, paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Sensitivity analysis\\data_sensitivity_analysis - ", min_non_NA, "\\multinom_results_nomed_", outcome, "_2_no_transformation_newref.xlsx"))
})