...
lapply(c('lems', 'uems', 'ltscore', 'ppscore', 'modben'), function(outcome){
  ...
  data_outcome_full$sexcd <- as.factor(data_outcome_full$sexcd)
  data_outcome_full$asimpc01 <- as.factor(data_outcome_full$asimpc01)
  data_outcome_full$tx1_r <- factor(data_outcome_full$tx1_r, levels = c("P", "D1", "D2"))
  data_outcome_full$bmi_status <- as.factor(data_outcome_full$bmi_status)
  data_outcome_full$nligr1 <- as.factor(data_outcome_full$nligr1)
  ...
  mm <- lapply(c("lpa", "kmeans"), function(meth){
    ...
    write_xlsx(bind_rows(lapply(gg, function(x){x[[1]]})), paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Sensitivity analysis\\data_sensitivity_analysis - ", min_non_NA, "\\multinom_results_imputed_nomed_", outcome, "_", meth, "_2_probs.xlsx"))
    write_xlsx(bind_rows(lapply(gg, function(x){x[[2]]})), paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Sensitivity analysis\\data_sensitivity_analysis - ", min_non_NA, "\\multinom_results_imputed_med_", outcome, "_", meth, "_2_probs.xlsx"))
  })
  ...
  mm <- lapply(c("lpa", "kmeans"), function(meth){
    gg <- lapply(2:6, function(g){
      ...
      pnomed <- predict(model_nomed, newdata = grid_nomed, type='probs')
      pmed <- predict(model_med, newdata = grid_med, type='probs')
      if (g == 2){
        pnomed <- data.frame("1" = 1-pnomed, "2"=pnomed)
        pmed <- data.frame("1" = 1-pmed, "2"=pmed)
        names(pnomed) <- c("1", "2")
      ...
      new_cols <- as.character(1:max(unlist(data_outcome_imputed[paste0(meth, "-clusters-", g)])))
      if (length(colnames(pnomed)) != length(new_cols)){
        for (c in new_cols){
          if (!(c %in% colnames(pnomed))){
            pnomed[c] <- 0
          }
        }
        pnomed <- pnomed[,new_cols]
      }
      if (length(colnames(pmed)) != length(new_cols)){
        for (c in new_cols){
          if (!(c %in% colnames(pmed))){
            pmed[c] <- 0
          }
        }
        pmed <- pmed[,new_cols]
    ...
    write_xlsx(bind_rows(lapply(gg, function(x){x[[1]]})), paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Sensitivity analysis\\data_sensitivity_analysis - ", min_non_NA, "\\multinom_results_full_nomed_", outcome, "_", meth, "_2_probs.xlsx"))
    write_xlsx(bind_rows(lapply(gg, function(x){x[[2]]})), paste0("C:\\Users\\LAURA\\Documents\\ETHZ\\Master\\Master thesis\\SCI project\\code - final version - 2\\Sensitivity analysis\\data_sensitivity_analysis - ", min_non_NA, "\\multinom_results_full_med_", outcome, "_", meth, "_2_probs.xlsx"))
  })
})
